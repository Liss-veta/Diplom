                                    ...
                                    <header class="uk-comment-header uk-position-relative">
                                        <div class="otst uk-flex-middle" uk-grid>
                                            ...
                                            <div class="uk-width-expand margin-h4">
                                                ...
                                                <div v-if="sovet.privilege == 0">
                                                    <v-btn prepend-icon="mdi-check-bold" rounded="0" variant="tonal"
                                                        color="success"
                                                        @click.prevent="privilegeSovet(sovet.id, '1')">Добавить</v-btn>
                                                    <v-btn prepend-icon="mdi-trash-can" rounded="0" variant="tonal"
                                                        color="red"
                                                        @click.prevent="privilegeSovet(sovet.id, '2')">Отклонить</v-btn>
                                                </div>
                                                <p v-else class="text-red">Отклонено вами</p>
                                            </div>
                                        </div>
                                    </header>
                                </article>
                            ...
                            <div class="h-100 uk-position-relative" cover uk-slideshow="animation: fade">
                                ...
                                <div class="uk-position-center-right uk-position-small h-100 d-flex align-center">
                                    <ul class="uk-thumbnav uk-thumbnav-vertical" cover>
                                        <li v-for="(image, index) in sovet.arr_images" style="height:75px;" cover
                                            :key="image" :uk-slideshow-item="index">
...
export default {
    ...
    methods: {
        ...
        privilegeSovet(id, stat) {
            axios.post(`/api/admin/updateprivilege/${id}/sovet`, {
                'stat': stat
            }, {
                headers: {
                    'Authorization': 'Bearer ' + this.token,
                ...
                this.getSovets();
                if (response.data.message == 'Успешно отклонено') {
                    NC.spawnNote("up", response.data.message, '', ["Закрыть"])
                }
                else {
